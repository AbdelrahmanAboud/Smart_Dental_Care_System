...
class _AnalyticsState extends State<Analytics>
    ...
    try {
      ...
      for (var doc in appointmentsSnapshot.docs) {
        ...
        if (data['patientId'] != null) {
          uniquePatients.add(data['patientId'].toString());
        }

        if (data['date'] != null) {
          DateTime appointmentDate = (data['date'] as Timestamp).toDate();

          if (appointmentDate.isAfter(startOfThisWeek) ||
              appointmentDate.isAtSameMomentAs(startOfThisWeek)) {
            String dayName = dayNames[appointmentDate.weekday - 1];
            weekMap[dayName] = (weekMap[dayName] ?? 0) + 1;
          }
        }

        String rawTreatment = (data['treatment'] ?? data['type'] ?? "")
            .toString()
            .trim();
        if (rawTreatment.contains("Filling")) {
          issuesMap["Filling"] = issuesMap["Filling"]! + 1;
        } else if (rawTreatment.contains("Extraction")) {
          issuesMap["Extraction"] = issuesMap["Extraction"]! + 1;
        } else if (rawTreatment.contains("Root Canal")) {
          issuesMap["Root Canal"] = issuesMap["Root Canal"]! + 1;
        } else if (rawTreatment.contains("Cleaning")) {
          issuesMap["Cleaning"] = issuesMap["Cleaning"]! + 1;
        }
      ...
      final reviewsSnapshot = await FirebaseFirestore.instance
          ...
          .where('doctorId', isEqualTo: doctorId)
          .get();
      double sumRating = reviewsSnapshot.docs.fold(
        0,
      ...
      if (mounted) {
        setState(() {
          ...
          avgRiskScore = scoredPatients == 0
              ? 0
              : totalRiskSum / scoredPatients;
          avgRating = reviewsSnapshot.docs.isEmpty
              ? 0
              : sumRating / reviewsSnapshot.docs.length;
          lowRiskCount = lCount;
          midRiskCount = mCount;
          hRiskCount = hCount;
          weeklyActivity = weekMap;
          dentalIssues = issuesMap;
          emergencyTotal = emergencySnapshot.docs.length;
          emergencyAccepted = acc;
          emergencyRejected = rej;
          isLoading = false;
        });
      }
    } catch (e) {
                  ...
                  GridView.count(
                    ...
                    children: [
                      ...
                      AnalyticsCard(
                        ...
                        iconColor: const Color(0xFF4CAF50),
                        value: avgRiskScore.toStringAsFixed(0),
  ...
  Widget _buildWeeklyBarChart() {
    final dayNames = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
    double maxVal = weeklyActivity.values
              ...
              BarChartData(
                ...
                barGroups: dayNames
                    ...
                    .map(
                      (e) => _neonBar(
                        e.key,
                        (weeklyActivity[e.value] ?? 0).toDouble(),
                      ),
                    )
                    .toList(),
                  ...
                  child: Column(
                    ...
                    children: [
                      _legendDot(
                        Colors.cyanAccent,
                        "Filling",
                        "${dentalIssues['Filling']}",
                      ),
                      _legendDot(
                        Colors.pinkAccent,
                        "Extraction",
                        "${dentalIssues['Extraction']}",
                      ),
                      _legendDot(
                        Colors.orangeAccent,
                        "Root Canal",
                        "${dentalIssues['Root Canal']}",
                      ),
                      _legendDot(
                        Colors.purpleAccent,
                        "Cleaning",
                        "${dentalIssues['Cleaning']}",
                      ),
                    ],
                  ),
                ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          RiskRangeBar(
            range: "80-100 (Low Risk)",
            count: lowRiskCount,
            color: Colors.greenAccent,
            maxCount: safeMax,
          ),
          RiskRangeBar(
            range: "60-79 (Moderate)",
            count: midRiskCount,
