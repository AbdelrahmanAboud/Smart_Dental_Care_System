...
class _AnalyticsReceptionistState extends State<AnalyticsReceptionist>
                    ...
                    return StreamBuilder<QuerySnapshot>(
                      ...
                      builder: (context, emergencySnapshot) {
                        ...
                        if (invoiceSnapshot.hasData) {
                          for (var doc in invoiceSnapshot.data!.docs) {
                            ...
                            if (data['status']?.toString().toLowerCase() ==
                                'paid') {
                              double amount = (data['totalAmount'] ?? 0)
                                  .toDouble();
                        ...
                        if (emergencySnapshot.hasData) {
                          emergencyCount = emergencySnapshot.data!.docs.where((
                            doc,
                          ) {
                            var data = doc.data() as Map<String, dynamic>;
                            Timestamp? ts = data['timestamp'];
                        ...
                        if (reviewSnapshot.hasData &&
                          ...
                          positivePercent =
                              (pos / reviewSnapshot.data!.docs.length) * 100;
                        }
                        ...
                        if (userSnapshot.hasData) {
                          for (var doc in userSnapshot.data!.docs) {
                            var data = doc.data() as Map<String, dynamic>;
                            Timestamp? ts =
                                data['pushedAt'] ?? data['createdAt'];
                            if (ts != null) {
                              DateTime date = ts.toDate();
                              if (date.month == now.month &&
                                  date.year == now.year) {
                                int week = ((date.day - 1) / 7).floor().clamp(
                                  0,
                                  4,
                                );
                                if (data['status']?.toString().toLowerCase() ==
                                    'done')
                                  revisitPatientsWeek[week]++;
                                else
                                  newPatientsWeek[week]++;
                              }
                            }
                          }
                        }
                                ...
                                GridView.count(
                                  ...
                                  children: [
                                    AnalyticsCard(
                                      icon: Icons.group,
                                      iconColor:
                                          AnalyticsReceptionist.primaryBlue,
                                      value:
      ...
      child: Column(
        children: [
                ...
                return BarChart(
                  BarChartData(
                    ...
                    barGroups: List.generate(
                      ...
                      (i) => _compareBars(
                        i,
                        newData[i] * _animation.value,
                        revisitData[i] * _animation.value,
                      ),
                    ),
                  ),
                );
          ...
          Row(
            ...
            children: [
              _legend(const Color(0xFF00FFA3), "New"),
              const SizedBox(width: 20),
              _legend(const Color(0xFF7C4DFF), "Revisit"),
            ],
          ),
        ],
      ),
                ...
                lineBarsData: [
                  LineChartBarData(
                    ...
                    belowBarData: BarAreaData(
                      show: true,
                      color: AnalyticsReceptionist.primaryBlue.withOpacity(0.1),
                    ),
                  ),
                ],
              ),
  ...
  Widget _buildSentimentPie(int p, int n, int nu) {
    ...
    String pP = total == 0
        ? "0%"
        : "${((p / total) * 100).toStringAsFixed(0)}%";
    String nP = total == 0
        ? "0%"
        : "${((n / total) * 100).toStringAsFixed(0)}%";
    String nuP = total == 0
        ? "0%"
        : "${((nu / total) * 100).toStringAsFixed(0)}%";
                ...
                Expanded(
                  child: PieChart(
                    PieChartData(
                      ...
                      sections: [
                        PieChartSectionData(
                          value: (p == 0 ? 1 : p).toDouble(),
                          color: Colors.cyanAccent,
                          radius: 20,
                          showTitle: false,
                        ),
                        PieChartSectionData(
                          value: (n == 0 ? 1 : n).toDouble(),
                          color: Colors.pinkAccent,
                          radius: 20,
                          showTitle: false,
                        ),
                        PieChartSectionData(
                          value: (nu == 0 ? 1 : nu).toDouble(),
                          color: Colors.amber,
                          radius: 20,
                          showTitle: false,
                        ),
  ...
  Widget _legendDot(Color c, String t, String v) => Padding(
  ...
  BarChartGroupData _compareBars(int x, double y1, double y2) =>
      BarChartGroupData(
        x: x,
        barRods: [
          BarChartRodData(
